...
async function request<T>(path: string, init?: RequestInit, fetcher: typeof fetch = fetch): Promise<T> {
  const response = await fetcher(path, { ...init, cache: 'no-store', headers: { accept: 'application/json', ...(init?.body ? { 'content-type': 'application/json' } : {}), ...init?.headers } })
...
export async function addManagedUser(input: { email: string; name: string; role: 'admin' | 'picker' | 'replenisher' }, fetcher: typeof fetch = fetch) {
