...
export function ReportsQueue({ role }: { role: AppRole }) {
  ...
  const reload = async () => { try { setReports(await getReports()); setError(''); window.dispatchEvent(new Event('warehouse:reports-updated')) } catch (reason) { setError(String(reason)) } }
  useEffect(() => { void reload(); const timer = setInterval(() => void reload(), 15000); return () => clearInterval(timer) }, [])
