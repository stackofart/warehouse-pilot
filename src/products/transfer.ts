...
export const productJsonSchema = {
  ...
  '$defs': {
    ...
    product: { type: 'object', required: ['sku', 'barcode', 'name', 'location'], properties: { sku: { type: 'string', pattern: '^\\d{3,10}$' }, barcode: { type: 'string', pattern: '^\\d{8,14}$' }, name: { type: 'string', minLength: 1 }, location: { type: 'string', minLength: 1 }, brand: { type: 'string' }, description: { type: 'string', maxLength: 2000, description: 'Необязательное подробное описание товара.' }, netContent: { type: 'string', description: 'Маркированное содержимое одной единицы, например 90 г или 700 мл.' }, verificationStatus: { type: 'string', enum: ['verified', 'unverified'] }, unitsPerBox: { type: 'number', exclusiveMinimum: 0 }, caseBarcode: { type: 'string', pattern: '^\\d{8,14}$' }, itemSpec: { '$ref': '#/$defs/physicalSpec' }, boxSpec: { allOf: [{ '$ref': '#/$defs/physicalSpec' }, { type: 'object', properties: { maxTopLoadKg: { type: 'number', exclusiveMinimum: 0 } } }] }, rigidity: { type: 'number', minimum: 1, maximum: 5 }, fragility: { type: 'number', minimum: 1, maximum: 5 }, technicalDataSource: { type: 'string', enum: ['manual', 'simulated', 'imported', 'web'] }, technicalVerificationStatus: { type: 'string', enum: ['not_searched', 'needs_review', 'partially_verified', 'verified', 'insufficient_data', 'conflict'] }, research: { type: 'object', description: 'Последний результат интернет-сверки со ссылками на источники.' }, imageDataUrl: { type: 'string' } } },
  },
}
...
export const productImportExample: ProductDocument = {
  ...
  products: [{ sku: '1511', barcode: '7290121920285', name: 'Название товара', location: '23.F', brand: 'Бренд', description: 'Необязательное описание: вкус, упаковка, особенности хранения и другие заметки.', netContent: '90 г', unitsPerBox: 24, caseBarcode: '17290121920282', itemSpec: { lengthCm: 16, widthCm: 8, heightCm: 1, weightKg: .1 }, boxSpec: { lengthCm: 39, widthCm: 19, heightCm: 14, weightKg: 3.1, maxTopLoadKg: 25 }, rigidity: 3, fragility: 2 }],
}
