...
React/Vite-приложение для распознавания складских заказов, ведения справочника товаров, расчёта маршрута и компоновки паллет. Общий каталог товаров и роли хранятся в Cloudflare D1, а текущие заказы и прогресс пока остаются локально в IndexedDB. Распознавание доступно через OpenAI или локальный Tesseract.
...
Перед первым локальным запуском примените схему общей базы:

```bash
npx wrangler d1 migrations apply warehouse-pilot-db --local
```

На localhost Worker создаёт тестового администратора из `DEV_AUTH_EMAIL`. Роль задаётся `DEV_AUTH_ROLE`.
...
## Публикация на GitHub Pages (только старый автономный frontend)

GitHub Pages не запускает Worker, D1 и серверную проверку ролей. Для текущей версии с авторизацией используйте Cloudflare Workers; инструкция находится в `docs/cloudflare-auth-and-d1.md`. Раздел ниже оставлен только для старой статической сборки без общей базы.
...
Приложение регистрирует service worker только в production-сборке. После первого полного открытия интерфейс может открыться без сети, но авторизация, общая база товаров и поиск требуют соединения с Worker. Заказы и прогресс сборки пока сохраняются локально в IndexedDB и не синхронизируются между устройствами.
