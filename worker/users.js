...
const publicRow = (row) => ({
  ...
  role: row.operational_role || row.role,
  status: row.status,
...
export async function handleUsersRequest(request, env, actor) {
  ...
  if (url.pathname === '/api/admin/users') {
    if (request.method === 'GET') {
      const result = await env.DB.prepare('SELECT id, email, display_name, role, operational_role, status, created_at, updated_at FROM users ORDER BY display_name, email LIMIT 250').all()
    ...
    if (request.method === 'POST') {
      ...
      const operationalRole = parsed.body?.role === 'replenisher' ? 'replenisher' : null
      ...
      const row = { id: crypto.randomUUID(), email, display_name: name || email.split('@')[0], role, operational_role: operationalRole, status: 'active', created_at: now, updated_at: now }
      await env.DB.prepare('INSERT INTO users (id, email, display_name, role, operational_role, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
        .bind(row.id, row.email, row.display_name, row.role, operationalRole, row.status, row.created_at, row.updated_at).run()
  ...
  const parsed = await readJson(request)
  if (parsed.response) return parsed.response
  const role = parsed.body?.role === 'admin' ? 'admin' : ['picker', 'replenisher'].includes(parsed.body?.role) ? 'picker' : null
  ...
  const existing = await env.DB.prepare('SELECT id, email, display_name, role, operational_role, status, created_at, updated_at FROM users WHERE id = ? LIMIT 1').bind(match[1]).first()
  ...
  const now = new Date().toISOString()
  await env.DB.prepare('UPDATE users SET role = ?, operational_role = ?, status = ?, updated_at = ? WHERE id = ?')
    .bind(role || existing.role, role ? (parsed.body.role === 'replenisher' ? 'replenisher' : null) : existing.operational_role, status || existing.status, now, existing.id).run()
  const updated = { ...existing, role: role || existing.role, operational_role: role ? (parsed.body.role === 'replenisher' ? 'replenisher' : null) : existing.operational_role, status: status || existing.status, updated_at: now }
  await env.DB.prepare('INSERT INTO audit_log (id, actor_id, action, entity_type, entity_id, payload_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)')
